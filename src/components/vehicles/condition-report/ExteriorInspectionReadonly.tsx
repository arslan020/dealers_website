...
export default function ExteriorInspectionReadonly({ faults }: { faults: FaultPoint[] }) {
    ...
    return (
        <div className="bg-white border border-[#E2E8F0] rounded-xl mt-5 overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
                <p className="text-[13px] font-semibold text-slate-800">Exterior Inspection</p>
            </div>
            <div className="px-5 py-4 space-y-6">
                {sorted.length === 0 ? (
                    <p className="text-[12px] text-slate-400 italic">No exterior faults recorded.</p>
                ) : (
                    <>
                        <div
                            ref={wrapRef}
                        ...
                        >
                            <img
                                src="/condition-report-exterior.png"
                                alt="Vehicle exterior diagram"
                                className="block w-full h-auto min-w-[min(100%,1332px)]"
                            />
                            <svg
                        ...
                        <div className="space-y-6">
                            {sorted.map((f, i) => {
                                ...
                                return (
                                    <div key={`row-${n}-${i}`} className="border-b border-slate-100 pb-6 last:border-0 last:pb-0">
                                        <div className="flex items-start gap-3">
                                            <span
                                                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#dc2626] text-[13px] font-bold text-white"
